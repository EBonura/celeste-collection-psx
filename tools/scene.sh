...
cargo run -q --release --manifest-path tools/psx-audio-capture/Cargo.toml --bin frametest -- --disc "dist/${GAME}_scene.cue" --out /tmp/p8out/scene_psx.ppm --frames "$((PSXFRAMES + 27))" >/dev/null 2>&1
